...
public class GamePlaningRepository {
    ...
    public List<GameResponse> getGamesForRound(Long roundId) {
        ...
        String query = "SELECT HOST.CLUB_NAME as hostName, HOST.CLUB_LOGO as hostLogo , HOST.CLUB_ID as hostId, VISITOR.CLUB_NAME as visitorName, VISITOR.CLUB_LOGO as visitorLogo," +
                " VISITOR.CLUB_ID as visitorId, G.game_date as gameDate, G.game_id as gameId" +
        ...
        List<GameResponse> gameResponses = jdbcTemplate.query(query,
                (rs, rowNum) -> new GameResponse(rs.getString("hostName"), rs.getString("hostLogo"),
                        ...
                        rs.getLong("visitorId"), rs.getDate("gameDate"), rs.getLong("gameId")));
