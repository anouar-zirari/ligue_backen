...
public class PlayerPlaningRepository {
    ...
    public List<PlayerResponse> getPlayerForClub(Long id){
        String query = "\n" +
                "SELECT P.player_first_name as playerFirstName, P.player_last_name as PlayerLastName, P.player_shirt_number as playerShirtNumber FROM PLAYER P INNER JOIN CLUB C ON P.CLUB_ID = C.CLUB_ID WHERE C.CLUB_ID = " + id;
